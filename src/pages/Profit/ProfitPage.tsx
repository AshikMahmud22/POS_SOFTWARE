...
import { getProfitEntries, deleteProfitEntry, IProfitEntry } from "../../services/profitService";
...
const ProfitPage: React.FC = () => {
  ...
  const filterRef = useRef({ month: "", year: "" });
  ...
  useEffect(() => {
    ...
    debounceRef.current = setTimeout(() => {
      setCurrentPage(1);
      fetchEntries(1, search, filterRef.current);
    }, 300);
  ...
  const handleYearChange = (year: string) => {
    ...
    setCurrentPage(1);
    fetchEntries(1, searchRef.current, newFilter);
  };

  const handleMonthChange = (month: string) => {
    const newFilter = { ...filterRef.current, month };
    setFilter(newFilter);
    filterRef.current = newFilter;
    setCurrentPage(1);
    fetchEntries(1, searchRef.current, newFilter);
  ...
  const handlePageChange = (page: number) => {
    setCurrentPage(page);
    fetchEntries(page, searchRef.current, filterRef.current);
  };
  ...
  const handleAfterSave = () => {
    setCurrentPage(1);
    fetchEntries(1, searchRef.current, filterRef.current);
  };
  ...
  return (
    <div className="md:p-8 mt-10 min-h-screen">
      ...
      {initialLoading ? (
        ...
        <>
          <ProfitTable entries={entries} onEdit={handleEdit} onDelete={handleDelete} />
